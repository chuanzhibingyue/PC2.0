 ...
 **/
(function(uc){
    uc.modules.chat.MeetingDetailChat = function(moduleId){
                this.bEnableInput = true;
                this.moduleId = moduleId;
            };
                jQuery.extend(uc.modules.chat.MeetingDetailChat.prototype, {
        ...
        OnMeetingMessageSent: function(errorCode, groupId, eventId, meetingStartTime, msgId, msgSeq, msgTime){
            ...
            if (errorCode == 0) {
                var tstr = uc.util.TimeUtil.getMessageTimeString(new Date(msgSeq), '', uc.settings.meeting.timeZone.offset);
        ...
        noticeMeetingDocumentUploaded: function(errorCode, eventId, operatorId, document, msgId){
            if (this.msgBox == null) {
                return;
            }
            if (errorCode == 0) {
                var tstr = uc.util.TimeUtil.getMessageTimeString(new Date(document.messageSeq), '', uc.settings.meeting.timeZone.offset);
                this.msgBox.updateMsgTime(msgId, tstr);
                this.msgBox.showSendFailFlag(msgId, false);
                this.msgBox.updateMessageSeq(msgId,document.messageSeq);
            } else {
                this.msgBox.showSendFailFlag(msgId, true);
            }

            if (errorCode == 0 && this.msgBox != null){
                this.msgBox.onFileIdUpdate(msgId, document.fileId, document.messageSeq, document.fileUrl);
            }
        },
