...
{   
	...
	std::wstring dumpPath = std::wstring(path) + L"\\Quanshi\\Bee\\dump\\";
	SHCreateDirectoryEx(NULL, dumpPath.c_str(), NULL);

	TCHAR fileName[64];
	SYSTEMTIME st;
	::GetLocalTime(&st);
	swprintf_s(fileName, L"Bee.Worker.exe.%04d-%02d-%02d-%02d-%02d-%02d-%02d-%02d.dmp", st.wYear, st.wMonth, st.wDay,
