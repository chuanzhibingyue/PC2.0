...
{   
	...
	std::wstring dumpPath = ucclient::FileUtil::GetAppDataPath() + L"\\Quanshi\\Bee\\dump\\";
	SHCreateDirectoryEx(NULL, dumpPath.c_str(), NULL);

	TCHAR fileName[64];
	SYSTEMTIME st;
	::GetLocalTime(&st);
	swprintf_s(fileName, L"Bee.%04d-%02d-%02d-%02d-%02d-%02d-%02d-%02d.dmp", st.wYear, st.wMonth, st.wDay,
		st.wHour, st.wMinute, st.wSecond, st.wMilliseconds, rand() % 100);
	std::wstring dumpFileFullPath = dumpPath + fileName;

	UC_LOG(ERROR) << "process crashed, dmp path: " << dumpPath.c_str();
	UC_LOG_FLUSH();

    // Create the minidump file
    hFile = CreateFile(
		dumpFileFullPath.c_str(),
        GENERIC_WRITE,
        0,
        NULL,
        CREATE_ALWAYS,
    ...
    BOOL bWriteDump = pfnMiniDumpWriteDump(
        ...
        dwProcessId,
        hFile,
		MINIDUMP_TYPE(MiniDumpWithDataSegs | MiniDumpWithProcessThreadData | MiniDumpWithIndirectlyReferencedMemory),
        &mei,
        NULL,
        &mci);
