...
#include "browser/drag_drop/bytes_write_handler.h"
...
CefMouseEvent ToMouseEvent(POINTL p, DWORD key_state, HWND hWnd) {
  ...
  ev.x = screen_point.x;
  ev.y = screen_point.y;
  ev.modifiers = OsrUtil::GetCefMouseModifiers(key_state);
  return ev;
}
...
void GetStorageForFileDescriptor(STGMEDIUM* storage,
  ...
  wcsncpy_s(descriptor->fgd[0].cFileName, MAX_PATH, file_name.c_str(), (std::min)(file_name.size(), static_cast<size_t>(MAX_PATH - 1u)));
