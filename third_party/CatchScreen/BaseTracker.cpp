...
BaseTracker::BaseTracker(LPCRECT lpSrcRect, UINT nStyle)
{
...
void BaseTracker::Draw(CDC* pDC) const
{
	...
	if ((m_nStyle & (dottedLine|solidLine)) != 0)
	{
		...
		rect.InflateRect(+1, +1);   // borders are tow pixels outside
	...
	// draw resize handles
	if ((m_nStyle & (resizeInside | resizeOutside)) != 0 && m_bAllowDrag)
	{
		...
		for (int i = 0; i < 8; ++i)
		{
			...
			{
				CRect rcHandle;
				GetHandleRect((TrackerHit)i, &rcHandle);

				//减少绘制的圆点区域。 这样UI上好看，同时让用户容易进入圆点（生效区域比显示区域大）
				...
				Pen pen(Color(GetRValue(Color::WhiteSmoke), GetGValue(Color::WhiteSmoke), GetBValue(Color::WhiteSmoke)), REAL(2));
				graphics.FillEllipse(&SolidBrush(Color::White), rcHandle.left, rcHandle.top, rcHandle.Width(), rcHandle.Height());
				graphics.DrawEllipse(&pen, rcHandle.left, rcHandle.top, rcHandle.Width(), rcHandle.Height());
			}
		}
	}
...
BOOL BaseTracker::SetCursor(CWnd* pWnd, UINT nHitTest) const
{
...
int BaseTracker::HitTest(CPoint point) const
{
...
int BaseTracker::NormalizeHit(int nHandle) const
{
...
BOOL BaseTracker::Track(CWnd* pWnd, CPoint point, BOOL bAllowInvert,
					   CWnd* pWndClipTo)
{
...
BOOL BaseTracker::TrackRubberBand(CWnd* pWnd, CPoint point, BOOL bAllowInvert)
{
...
void BaseTracker::DrawTrackerRect(
								 LPCRECT lpRect, CWnd* pWndClipTo, CDC* pDC, CWnd* pWnd)
{
	...
	*/
	//此函数是画调整大小和位置时画虚线
	((CCatchScreenDlg *)pWnd)->Invalidate();

}

void BaseTracker::AdjustRect(int nHandle, LPRECT)
{
...
void BaseTracker::GetTrueRect(LPRECT lpTrueRect) const
{
...
void BaseTracker::OnChangedRect(const CRect& /*rectOld*/)
{
...
void BaseTracker::GetHandleRect(int nHandle, CRect* pHandleRect) const
{
	ASSERT(nHandle < 8);
...
int BaseTracker::GetHandleSize(LPCRECT lpRect) const
{
	if (lpRect == NULL)
		lpRect = &m_rect;
	...
	return LogicalToDevice(size + POINT_SIZE_OFFSET);
}

int BaseTracker::HitTestHandles(CPoint point) const
{
	CRect rect;
	UINT mask = GetHandleMask();
...
BOOL BaseTracker::TrackHandle(int nHandle, CWnd* pWnd, CPoint point,
							 CWnd* pWndClipTo)
{    
	ASSERT(nHandle >= 0);
	...
	for (;;)
	{
		...
		case WM_MOUSEMOVE:
			...
			// dispatch the point(right and bottom) to the main window to update the pos of the toolbat
			::SendMessage(m_hWnd, WM_UPDATE_TOOLBAR, (WPARAM)&CPoint(m_rect.left,m_rect.top), (LPARAM)&CPoint(m_rect.right,m_rect.bottom) );
...
ExitLoop:
	...
	::SendMessage(m_hWnd, WM_DRAG_END, 0, 0);

	// return TRUE only if rect has changed
	return !rectSave.EqualRect(&m_rect);
}
void BaseTracker::GetModifyPointers(int nHandle, int** ppx, int** ppy, int* px, int* py)
{
	ASSERT(nHandle >= 0);
	ASSERT(nHandle <= 8);
...
UINT BaseTracker::GetHandleMask() const
{
...
void BaseTracker::SetRectColor(COLORREF rectColor)
{
	m_rectColor=rectColor;
	CreatePen();
}

void BaseTracker::CreatePen()
{
...
void BaseTracker::SetResizeCursor(UINT nID_N_S,UINT nID_W_E,UINT nID_NW_SE,UINT nID_NE_SW,UINT nIDMiddle)
{
	//////////////////////////////////////////////////////////////////////////////////
	//                             N            
	//               NW -----------|------------NE
	...
	_afxCursors[0] = ::LoadCursor(NULL,MAKEINTRESOURCE(nID_NW_SE) );  //nw
	_afxCursors[1] = ::LoadCursor(NULL,MAKEINTRESOURCE(nID_NE_SW) );  //ne
	_afxCursors[2] = _afxCursors[0];						//se	
	_afxCursors[3] = _afxCursors[1];						//sw
	_afxCursors[4] = ::LoadCursor(NULL,MAKEINTRESOURCE(nID_N_S) );  //n
	_afxCursors[5] = ::LoadCursor(NULL,MAKEINTRESOURCE(nID_W_E) );  //w
	_afxCursors[6] = _afxCursors[4];						//s
	_afxCursors[7] = _afxCursors[5];                        //e
	_afxCursors[8] = ::LoadCursor(NULL,MAKEINTRESOURCE(nIDMiddle) ); //m

}

BOOL BaseTracker::SetHandler(HWND hwnd)
{
