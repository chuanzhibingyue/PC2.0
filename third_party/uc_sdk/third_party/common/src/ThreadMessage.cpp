...
public:
    void FireAll (MessageProc *proc) {
        ...
        std::queue<ThreadMessage> queue_copy;
        _msgs.swap(queue_copy);
        pthread_mutex_unlock(&m_mutex);
        
        while (!queue_copy.empty()) {
            ThreadMessage msg = queue_copy.front();
            queue_copy.pop();
...
public:
    ...
    bool AttachMainThreadMessageProc(MessageProc *proc) {
        WNDCLASSA wc = {0, WndProc, 0, 0, 0, 0, 0, 0, 0, "uc_thread_message"};
        ...
        if (!::IsWindow(_hwnd)) {
            _hwnd = ::CreateWindowA((const char *)atom, "uc_thread_message_window",
    ...
    void DetachMainThreadMessageProc() {
        ...
        UnregisterClassA("uc_thread_message", NULL);
    }
