...
sockaddr const* const* url::get_ips(bool usedns) {
	...
	UC_LOG(INFO) << "url::get_ips:(m_host):" << m_host;

	if (!getaddrinfo(m_host.c_str(), NULL, usedns ? NULL : &flags, &info)) {
		struct addrinfo* first = info;
		unsigned int i = 0;

		UC_LOG(INFO) << "!getaddrinfo(m_host.c_str(), NULL, usedns ? NULL : &flags, &info)";
		...
		if (i == 0)
		{
			UC_LOG(INFO) << "url::get_ips, m_ips = " << m_ips;
			return m_ips = NULL;
		}
...
char* url::get_pac() {
	...
	for (int i=0 ; m_ips[i] ; i++) {
		...
		{
			UC_LOG(INFO) << "connect success, IPV4 ";
			break;
		}
		else if (m_ips[i]->sa_family == AF_INET6 &&
			!connect(sock, m_ips[i], sizeof(struct sockaddr_in6)))
		{
			UC_LOG(INFO) << "connect success, IPV6 " ;
			break;
		}

		UC_LOG(INFO) << "connect failed: error code:" << ::GetLastError();
		shutdown(sock, SHUT_RDWR);
		sock = -1;
	}
	...
	UC_LOG(INFO) << "request:" << request << " , size:" << request.size();

	// Send HTTP request
	size_t return_value = send(sock, request.c_str(), request.size(), 0) ;
	if (return_value != request.size())
	{
		UC_LOG(INFO) << "sent return:" << return_value;
		shutdown(sock, SHUT_RDWR);
		return NULL;
	}
...
// 	}
	...
	UC_LOG(INFO) << "recvline() , line:" << line;
	...
	if (sscanf(line.c_str(), "HTTP/1.%*d %lu", &status) == 1 && status == 200) {
		...
		for (line = recvline(sock) ; line != "\r" && line != "" ; line = recvline(sock)) {
			UC_LOG(INFO) << "in for loop: recvline: " << line;
			...
			else if (content_length == 0)
			{
				...
				UC_LOG(INFO) << "line for content-length:" << content_length;
			}
		}
		...
		do {
			...
			if (chunked) {
				...
				if (sscanf(recvline(sock).c_str(), "%x", &chunk_length) != 1 || chunk_length == 0)
				{
					UC_LOG(INFO) << "Get the chunk-length line as an integer: " << chunk_length;
					break;
				}
			...
			if (content_length >= PAC_MAX_SIZE)
			{
				UC_LOG(INFO) << "content_length  : " << content_length << " > 102400";
				break;
			}

			if (content_length == 0)
			{
				content_length = PAC_MAX_SIZE - 1;
			}
		...
		UC_LOG(INFO) << "buffer: " << buffer << " , content_length:" << content_length << " , buffer size:" << string(buffer).size();
		if (string(buffer).size() != content_length && (content_length !=( PAC_MAX_SIZE -1))) {
			delete[] buffer;
			buffer = NULL;
		}
	}
