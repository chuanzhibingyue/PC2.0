...
void proxy_factory::get_config(url &realurl, vector<url> &config, string &ignore) {
	...
	for (vector<config_extension*>::iterator i=configs.begin() ; i != configs.end() ; i++) {
		...
		UC_LOG(INFO) << "proxy_factory::get_config realurl:" << realurl.to_string();
		...
		try {
			...
			for (auto item : config)
				UC_LOG(INFO) << "proxy_factory::get_config: config:" << item.to_string();

			/*if (debug)*/ {
				if (configurator) {
// 					cerr << "Configuration extension is: " << typeid(*configurator).name() << endl;
// 					cerr << "Ingored list is: " << ignore << endl;
					UC_LOG(INFO) << "Configuration extension is: " << typeid(*configurator).name();
					UC_LOG(INFO) << "Ingored list is: " << ignore;
				} else {
//					cerr << "No configuration extension found." << endl;
					UC_LOG(INFO) << "No configuration extension found.";
				}
			}
			break;
		}
...
bool proxy_factory::expand_pac(url &confurl)
{
	...
	if (confurl.get_scheme().substr(0, 4) == "pac+") {
		...
		if (this->pac) {
			UC_LOG(INFO) << "proxy_factory::expand_pac, (this->pacurl):" << this->pacurl->to_string() << " , (confurl):" << confurl.to_string();
		...
		if (!this->pac) {
			...
			if (debug) {
				if (!this->pac)
				{
					...
					UC_LOG(INFO) << "Unable to download PAC!";
				}
				else
				{
				//	cerr << "PAC received!" << endl;
					UC_LOG(INFO) << "PAC received!";
				}
			}
		}
	}
...
void proxy_factory::run_pac(url &realurl, const url &confurl, vector<string> &response) {
	...
	UC_LOG(INFO) << "confurl.get_scheme()" << confurl.get_scheme() << ", confurl.get_scheme().substr(0,4)" << confurl.get_scheme().substr(0, 4);
	if (this->pac && (confurl.get_scheme() == "wpad" || confurl.get_scheme().substr(0, 4) == "pac+") ) {
		try
		{
			pacparser_parse_pac_string(this->pac);
			UC_LOG(INFO) << "pacparser_parse_pac_string:(this->pac): " << this->pac;
// 
// 			char hname[128];
// 			struct hostent *hent;
...
// 			}
			...
			char* pacresp = pacparser_find_proxy(realurl.to_string().c_str(), realurl.get_host().c_str());
			UC_LOG(INFO) << "pacparser_find_proxy: (realurl):" << realurl.to_string() << ", (pacresp):" << pacresp;
			if (pacresp)
			{
				format_pac_response(string(pacresp), response);

				for (auto item: response)
					UC_LOG(INFO) << "format_pac_response:response:" << item.c_str() ;
			}
		}
		catch (std::exception&)
		{
			//cerr << "Parser PAC content failed!" << endl;
			UC_LOG(INFO) << "Parser PAC content failed!";
		}
	}
}
