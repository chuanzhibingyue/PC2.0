...
#include <math.h>
#include "./decoder/AMRFileDecoder.h"
...
/// buffering
#define BLOCK_SIZE           4000       // queue block size
...
DWORD WavePlayer::GetAudioLength(LPCTSTR lpszFile)
{
    // 设置解码器
    if(m_pDecoder == NULL)
    {
...
BOOL WavePlayer::WaveOutProcImpl(HWAVEOUT hwo, UINT uMsg, DWORD dwParam1, DWORD dwParam2)
{
    ...
    if(uMsg == WOM_DONE)
    {
		...
		if (m_soundCB)
		{
			WAVEFORMATEX wfmtx = m_pDecoder->GetWaveFromatX();
			int interval = (BLOCK_SIZE * 1000) / wfmtx.nAvgBytesPerSec;
			int total = ceil(float(m_pDecoder->GetTimeLength()) / interval);
			int percent = ++m_notification * 100 / total;
			m_soundCB->onPlayingNotification(percent > 100 ? 100 : percent, (float)m_pDecoder->GetTimeLength() / 1000, m_cbData);
		}
		LeaveCriticalSection(&m_cs);
    }
...
void WavePlayer::SetCallback(ISoundCallBack *psoundCB, long cbData)
{
	m_soundCB = psoundCB;
	m_cbData = cbData;
}
...
void WavePlayer::OnPlayFinished()
{
	...
	if (m_soundCB)
		m_soundCB->onSoundPlayed(m_cbData);
}